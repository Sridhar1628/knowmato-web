    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 py-10 sm:px-6 lg:px-8">
          ...
          {Array.from({ length: sectionCount }).map((_, idx) => (
            ...
            >
              <h2 className="text-xl font-bold text-white mb-3">
                {t(`communityGuidelines.section${idx + 1}.title`)}
              </h2>
              <div className="text-white/80 whitespace-pre-line leading-relaxed text-sm sm:text-base">
                {t(`communityGuidelines.section${idx + 1}.content`)}
              </div>
            </div>
          ))}
        </div>
        ...
        <p className="mt-8 text-center text-xs text-white/30">
          {t("communityGuidelines.copyright", { year: currentYear })}
        </p>
      </div>
    </div>
  );
