...
export default function AIUsagePolicyPage() {
  ...
  // Build sections from translation keys (1‑based index)
  const sections = Array.from({ length: 10 }, (_, i) => ({
    title: t(`aiPolicy.section${i + 1}.title`),
    content: t(`aiPolicy.section${i + 1}.content`),
  }));
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 py-10 sm:px-6 lg:px-8">
        ...
        <p className="mt-8 text-center text-xs text-white/30">
          {t("aiPolicy.copyright", { year: new Date().getFullYear() })}
        </p>
      </div>
    </div>
  );
