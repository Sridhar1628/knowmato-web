  ...
  useEffect(() => {
    const fetchEnrollments = async () => {
      try {
        ...
        setError(err?.response?.data?.detail || err?.message || t('myCourses.loadError'));
    ...
    return (
      <span
        className={`rounded-full px-2 py-0.5 text-xs capitalize ${
          statusClasses[status] || 'bg-white/10 text-white/50'
        }`}
      >
    ...
    <div className="min-h-screen bg-[#0B0C10] p-6 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-10">
          ...
          <p className="mt-2 text-white/70">{t('myCourses.subtitle')}</p>
        </div>
        ...
        {!loading && !error && enrollments.length === 0 && (
          <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-12 text-center">
            <p className="text-lg text-white/50">{t('myCourses.empty')}</p>
            <button
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <span className="text-sm text-white/60">
                    {t('myCourses.progressComplete', { percent: enrollment.progress_percentage })}
                  </span>
                </div>
                ...
                {enrollment.last_lecture_title && (
                  <p className="mt-3 text-xs text-white/40">
                    {t('myCourses.lastLecture', { title: enrollment.last_lecture_title })}
                  </p>
                )}

                {/* Enrolled date */}
                <p className="mt-1 text-xs text-white/30">
                  {t('myCourses.enrolledOn', { date: formatDate(enrollment.enrolled_at) })}
                </p>
                ...
                <div className="mt-auto pt-4 flex gap-2">
                  ...
                  {(enrollment.status === 'cancelled' || enrollment.status === 'expired') && (
                    <div className="w-full text-center text-sm text-white/40 py-2 capitalize">
                      {t(`myCourses.status${enrollment.status.charAt(0).toUpperCase() + enrollment.status.slice(1)}`)}
                    </div>
                  )}
                </div>
              </div>
