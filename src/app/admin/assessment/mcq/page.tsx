...
import { getAllSamples, SampleQuestion } from "@/services/assessmentService";

export default function AdminMCQListPage() {
  const router = useRouter();

  const [mcqs, setMcqs] = useState<SampleQuestion[]>([]);
  const [loading, setLoading] = useState(true);

  const fetchMCQs = useCallback(async () => {
    try {
      const data = await getAllSamples(); // <-- you must create this service
      setMcqs(data || []);
    } catch (err) {
      ...
      <AdminLayout>
        <div className="flex min-h-[60vh] items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-white/60">Loading MCQs...</p>
          </div>
        </div>
      </AdminLayout>
    );
        ...
        <div className="relative z-10 p-4 sm:p-6 lg:p-8">
          ...
          >
            <div>
              ...
              <p className="text-white/70 mt-1">Manage multiple‑choice questions.</p>
            </div>
            <button
              onClick={() => router.push("/admin/assessment/mcq/create")}
          ...
          {mcqs.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-white/20 bg-white/5 backdrop-blur-xl p-12 text-center">
              <p className="text-white/50">No MCQs found. Create one to get started.</p>
            </div>
          ) : (
                ...
                >
                  <div className="mb-3 sm:mb-0 flex-1">
                    ...
                    <p className="text-xs text-white/50 mt-1">
                      Correct: {mcq.correct_answer} • Assignment:{" "}
                      {mcq.Assignment ? `#${mcq.Assignment}` : "None"}
                    </p>
