    ...
    if (!isNew) { 
      const fetchCourse = async () => { 
        ...
        try { 
          ...
          AlertService.error("Load Failed", "Failed to load course"); 
  ...
  const handleSubmit = async (e: React.FormEvent) => { 
    ...
    try { 
      if (isNew) { 
        ...
        AlertService.success("Course Created", "Course created successfully"); 
      } else { 
        await updateCourse(Number(id), form); 
        AlertService.success("Course Updated", "Course updated successfully"); 
      } 
      router.push("/admin/courses"); 
    } catch (err) { 
      AlertService.error("Save Failed", "Failed to save course"); 
    } finally { 
      setSaving(false); 
    } 
  }; 
