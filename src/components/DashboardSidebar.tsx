...
function NavItem({ icon, label, href, active, onClick }: NavItemProps) {
  ...
  return (
    <button
      onClick={() => {
        router.push(href);
        onClick?.(); // close mobile sidebar if needed
      }}
...
export default function DashboardSidebar({ open, onClose }: DashboardSidebarProps) {
  ...
  const sidebarContent = (
    <aside className="flex h-full w-64 flex-col bg-[#0f0f23] text-white">
      {/* Brand / Logo */}
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-4">
        {routes.map((route) => (
          <NavItem
            ...
            onClick={onClose} // close sidebar when any link is clicked on mobile
          />
        ))}
      </nav>
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 z-50 h-full transform transition-transform duration-300 ease-in-out md:static md:flex md:translate-x-0 ${
